...
class SQLManager:
    ...
    def _get_latest_stats(cur: sql.Cursor, uid):
        cur.execute('select * from user_stats where uid = {0} and time = (SELECT max(time) FROM tracks where uid = {0})'
                    .format(uid))
        r = cur.fetchone()
        st = ww6StatBotPlayer.PlayerStat
        stid = None
        if r:
            stid, uid, st.time, st.hp, st.attack, st.armor, st.power, st.accuracy, st.oratory, st.aggility, st.stamina = r
        return stid, st
    ...
    def update_stats(self, pl: ww6StatBotPlayer.Player):
        ...
        conn = sql.connect(self.database)
        cur = conn.cursor()
        cur.execute('INSERT into user_stats(uid, time, hp, attack, armor, power, accuracy, oratory, aggility, stamina) '
                    'values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                    (uid, st.time, st.hp, st.attack, st.power, st.accuracy, st.oratory, st.aggility, st.stamina))
        conn.close()
